...
rec {
  shellHook-for = { archRole, dry-wit, layer, nixpkgsRelease, org, package
    , python, pythoneda-shared-pythoneda-domain
    , pythoneda-shared-pythoneda-banner, banner, repo, space }:
    let
    ...
    in ''
      #!/usr/bin/env ${dry-wit}/dry-wit
      # fun: main
      # api: public
      # txt: Prints the banner and manipulates the PYTHONPATH variable.
      ...
      function main() {
        ...
        if isEmpty "$PYTHONEDA_ROOT_FOLDER"; then
          ...
          if isNotEmpty "$PYTHONEDA_EXTRA_NAMESPACES"; then
            local _oldIFS="$IFS";
            IFS="$DWIFS";
            local _namespace;
            for _namespace in "$(echo $PYTHONEDA_EXTRA_NAMESPACES | sed 's : \n g')"; do
              IFS="$_oldIFS";
              toUpper "$_namespace";
              _namespaceUpper="$RESULT";
              local _variable="$(echo -n \"PYTHONEDA_$_namespaceUpper\"; echo '_EXTRA_NAMESPACES')"
              if isEmpty "$(evalVar "$_variable")"; then
    ...
    '';
  devShell-for = { archRole, banner, dry-wit, layer, nixpkgsRelease, org
    , package, pkgs, python, pythoneda-shared-pythoneda-banner
